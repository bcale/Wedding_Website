/* ── Token persistence (runs first, before anything else) ───────────────── */
...
/* ── Home page only (RSVP toggles, modal, form) ─────────────────────────── */
...
if (btnYesCer) {
  ...
  function showGuestField() {
    guestField.style.display = (attending_cer || attending_rec) ? "" : "none";
  }
...
if (songSearch) {
  ...
  async function fetchSongs(q) {
    ...
    try {
      const res  = await fetch(
        `/api/search-songs?q=${encodeURIComponent(q)}&token=${encodeURIComponent(token)}`
      );
      const data = await res.json();
      renderResults(data);
    } catch {
  ...
  async function loadSongRequests() {
    ...
    try {
      const res  = await fetch(
        `/api/song-requests?token=${encodeURIComponent(token)}`
      );
      const data = await res.json();
      if (data.length) {
